...
The tooling landscape is evolving just as fast. Coding agents are no longer confined to terminal sessions on a developer's laptop. [Claude Code Web](https://code.claude.com/docs/en/claude-code-on-the-web) runs full agentic coding sessions in isolated cloud VMs, creating pull requests against your repository without requiring a local environment. Google's [Jules](https://jules.google/) takes a similar approach, cloning repositories to cloud VMs and executing multi-file changes autonomously with support for dozens of concurrent tasks. And projects like [Paperclip](https://docs.paperclip.ing/start/what-is-paperclip) are pushing further, providing a control plane that manages entire fleets of coding agents as an AI workforce with org structure, budgets, goal alignment, and governance gates. The direction is clear: coding agents are becoming infrastructure, not tools. As they run continuously and in parallel, quality must shift left continuously too. Every harness constraint, every security gate, every architectural rule moves earlier in the cycle, so defects are caught by the system before they reach a human. The tighter the harness, the more autonomy you can safely grant.
...
## Security Is Even More Paramount.
...
- [StrongDM's Attractor](https://factory.strongdm.ai/products/attractor)
- [Ethan Mollick observed](https://www.oneusefulthing.org/p/the-shape-of-the-thing)
